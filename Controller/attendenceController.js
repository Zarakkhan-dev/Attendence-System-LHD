...
export const checkInAttendance = catchAsync(async (req, res, next) => {
    ...
    if (!employeeDetails) return next(new appError('Employee details not found', 404));
    ...
    const updateFields = {
        totalLateDays: isLate ? employeeDetails.totalLateDays + 1 : employeeDetails.totalLateDays,
    };
    ...
    res.status(200).json({
        ...
        data: {
            checkInTime,
            status,
            message: isLate ? `Late by ${Math.round(differenceInMinutes - 16)} minutes` : "Attendance marked on time",
            day,
            month,
            year
        }
...
const id  = employee[0]._id
    ...
    const attendance = await Attendance.findOne( {employeeId:id , checkOutTime :null });
    if (!attendance) {
        return next(new appError('Check-in record not found. Please check in first.', 400));
    }
    ...
    const newTotalSalary = employeeDetail.salaryPerDay * employeeDetail.totalWorkingDays;
    ...
    await Employee_details.updateOne({ userId: id }, { 
